...
def schedules_to_jsonable():
    ...
    return {
        ...
        "working_hours": {b:(wh[0].isoformat(),wh[1].isoformat()) for b,wh in st.session_state.working_hours.items()},
        "schedules": data,
...
def load_state_from_json(filename="schedules.json"):
    ...
    def parse_time_str(t):
        ...
        except: return datetime.strptime(t,"%H:%M:%S").time() if ":" in t else datetime.strptime(t,"%H:%M").time()

    st.session_state.working_hours = {b:(parse_time_str(wh[0]),parse_time_str(wh[1])) for b,wh in data.get("working_hours",{}).items()}
...
def parse_slot_types(text):
    ...
    for line in text.splitlines():
        ...
        if len(parts)>=3:
            try: out.append({"name":parts[0].strip(),"minutes":int(parts[1]),"weight":float(parts[2])})
            except: pass
        elif len(parts)==2:
            try: out.append({"name":parts[0].strip(),"minutes":int(parts[1]),"weight":1})
            except: pass
    return out
...
def add_slot_to_brygada(brygada,day,slot):
    d=day.strftime("%Y-%m-%d")
    if brygada not in st.session_state.schedules: st.session_state.schedules[brygada]={}
    if d not in st.session_state.schedules[brygada]: st.session_state.schedules[brygada][d]=[]
...
def total_work_minutes_for_brygada(brygada):
    s,e=st.session_state.working_hours[brygada]
    return minutes_between(datetime.combine(date.today(),s),datetime.combine(date.today(),e))
...
def used_minutes_for_week(brygada,any_day_in_week):
    return sum(daily_used_minutes(b, d) for d in week_days_containing(any_day_in_week))

# ===================== PREDEFINED SLOTS =====================
PREFERRED_SLOTS={"8:00-12:00":(time(8,0),time(12,0)),
                 "12:00-16:00":(time(12,0),time(16,0)),
                 "16:00-20:00":(time(16,0),time(20,0))}

# ===================== HEURISTICS =====================
def find_best_insertion(client_name,slot_type_name,day,pref_range):
    ...
    pref_start,pref_end=PREFERRED_SLOTS[pref_range]
    ...
    for b in st.session_state.brygady:
        ...
        if pref_end_dt<=day_start_dt or pref_start_dt>=day_end_dt: continue
        slots=get_day_slots_for_brygada(b,day)
        intervals=[]
        if not slots: intervals.append((day_start_dt,day_end_dt))
        else:
            intervals.append((day_start_dt,slots[0]["start"]))
            for i in range(len(slots)-1): intervals.append((slots[i]["end"],slots[i+1]["start"]))
            intervals.append((slots[-1]["end"],day_end_dt))
        for iv_start,iv_end in intervals:
            if minutes_between(iv_start,iv_end)<dur_min: continue
            ...
            if candidate_start>latest_start: continue
...
def schedule_client(client_name,slot_type_name,day,pref_range):
    res=find_best_insertion(client_name,slot_type_name,day,pref_range)
    if not res: return False,None
    slot={"start":res["start"],"end":res["end"],"slot_type":slot_type_name,"duration_min":int((res["end"]-res["start"]).total_seconds()/60),"client":client_name,"pref_range":pref_range}
    add_slot_to_brygada(res["brygada"],day,slot)
    return True,res

# ===================== FULL UI =====================
st.title("📅 Harmonogram slotów z heurystyką")

with st.sidebar:
    st.subheader("⚙️ Konfiguracja")
    txt=st.text_area("Typy slotów (Nazwa, minuty, waga)",value="\n".join(f"{s['name']},{s['minutes']},{s.get('weight',1)}" for s in st.session_state.slot_types))
    ...
    b=st.slider("Balance",0.0,1.0,st.session_state.heur_weights["balance"])
    s=d+g+b
    if s>0: st.session_state.heur_weights={"delay":d/s,"gap":g/s,"balance":b/s}
    else: st.session_state.heur_weights={"delay":0.34,"gap":0.33,"balance":0.33}

    if "week_offset" not in st.session_state: st.session_state.week_offset=0
    st.sidebar.subheader("⬅️ Wybór tygodnia")
    col1,col2=st.sidebar.columns(2)
    if col1.button("‹ Poprzedni tydzień"): st.session_state.week_offset-=1
    if col2.button("Następny tydzień ›"): st.session_state.week_offset+=1
    week_ref=date.today()+timedelta(weeks=st.session_state.week_offset)
    week_days=week_days_containing(week_ref)
...
with st.form("add_client_form"):
    ...
    st.info(f"Automatycznie: {auto_type}, preferencja: {auto_pref}")
...
for b in st.session_state.brygady:
    for d in week_days:
        ...
        for s in st.session_state.schedules.get(b,{}).get(d_str,[]):
            all_slots.append({
